...
async function preloadCaches() {
    try {
        console.log('ΓÅ│ Preloading caches...');

        // Pricing settings ΓÇö cached for 30 minutes (matches PRICING_CACHE_TTL in controller)
        const pricing = await prisma.pricing_settings.findFirst();
        if (pricing) {
            cache.set('pricing_settings', pricing, 30 * 60);
            console.log('  Γ£à Pricing cache loaded');
        }

        // Stagger queries to avoid connection burst on startup
        await new Promise(r => setTimeout(r, 500));

        // Active fleet ΓÇö cached for 2 minutes
        const fleet = await prisma.fleet_vehicle.findMany({
            where: { is_active: true },
            orderBy: { category: 'asc' }
        });
        cache.set('fleet_vehicles_true', fleet, 120);
        console.log(`  Γ£à Active fleet cache loaded (${fleet.length} vehicles)`);

        await new Promise(r => setTimeout(r, 500));

        // All fleet vehicles ΓÇö cached for 2 minutes
        const allFleet = await prisma.fleet_vehicle.findMany({ orderBy: { category: 'asc' } });
        cache.set('fleet_vehicles_all', allFleet, 120);
        console.log(`  Γ£à All fleet cache loaded (${allFleet.length} vehicles)`);

        console.log('Γ£à All caches preloaded successfully');
    } catch (err) {
        console.warn('ΓÜá∩╕Å  Cache preload failed, will load on first request:', err.message);
    }
}
...
async function startServer() {
    ...
    try {
        ...
        console.log('Γ£à Database connection established.');

        // Preload caches AFTER warmup completes
        await preloadCaches();
    } catch (err) {
        console.error('Γ¥î Could not connect to database during startup. Starting server anyway...');
